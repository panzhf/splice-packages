<template>
  <view>
    ...
    >
      ...
      <view :style="{ paddingTop: top }" class="main flex-1">
        ...
        <view v-for="(item, index) in components" :key="index" class="component-item">
          ...
          <anti-fake-info
            ...
            :is-first="guideInfo.ScanCount === 1"
            :info="item.data"
            :code-data="codeData"
            :fw-tip="guideInfo.FwQueryTip"
          />
          <simple-anti-fake-info v-if="item.code === 'SimpleAntiFakeInfo'" :info="item.data" :code-data="codeData" />
          <not-guide-tag v-if="item.code === 'NotGuideTag'" :info="item.data" />
          <guide-btn
            v-if="item.code === 'ApplyGuideBtn'"
            :info="item.data"
            :guide-info="registerGuideInfo(1, item.data)"
          />
          <no-activate v-if="item.code === 'NoActivate'" :info="item.data" />
          <guide-btn v-if="item.code === 'GoActive'" :info="item.data" :guide-info="registerGuideInfo(2, item.data)" />
          <code-no-active
            v-if="item.code === 'CodeNoActive'"
            :info="item.data"
            :code-data="codeData"
            :msg="guideInfo.DGAwardNonactivatedMsg"
          />
      ...
      <view
        v-if="showRule && isScanPage && !isErrorDefalutModel"
        class="rule-icon"
        mode="widthFix"
        :style="{ top: rulesTop }"
        @click="handleClickRule"
      >
      ...
      <view v-show="isShowBottomBtns" class="bottom-btns">
        <template v-for="(item, index) in bottomBtns">
          <view :key="index">
            ...
            />
            <div
              v-if="item.imageUrl && (item.jumpType === 'mp' || item.sceneType === 'guideassistant')"
              class="bottom-btn"
      ...
      <nowin-dialog
        ...
        :custom-scan-configs="contentJson"
        :register-award-msg="guideInfo.DGAwardMessage"
        :is-new-custom="true"
      />
...
import SimpleAntiFakeInfo from 'packages/design/scan/simple-anti-fake-info.vue'
import NotGuideTag from 'packages/design/scan/not-guide-tag.vue'
import NoActivate from 'packages/design/scan/no-activate.vue'
import GuideBtn from 'packages/design/scan/guide-btn.vue'
import CodeNoActive from 'packages/design/scan/code-no-active.vue'
// 带参跳转
...
export default {
    ...
    contentJson(val) {
      ...
      if (data) {
        ...
        if (this.dgerrortype === 1) {
          ...
          this.handleComponents(data.NotGuidePage.page)
        } else if (this.dgerrortype === 3) {
          // 码未激活
          this.handleComponents(this.contentJson.CodeActivatePage.page)
        } else {
  ...
  methods: {
    registerGuideInfo(type, data) {
      if (!data.tipsText) {
        data.tipsText = type === 1 ? '申请成为导购员' : '前往激活导购员'
      }
      let scene = encodeURIComponent(`m=${this.codeData.memberlogin}&t=${type}`)
      return {
        appid: this.mpMap.guideassistant,
        path: `/pages/register/index?scene=${scene}`,
        h5link: '',
        scene: 'guideassistant'
      }
    },
