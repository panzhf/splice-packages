    ...
    <view v-if="isShow" class="win-dialog-container" :style="{ 'z-index': zIndex }">
      ...
      <view class="content">
        ...
        <view class="pop__content" :style="{ backgroundImage: `url(${popBg})` }">
          <image class="pop__bg-top-img" :src="popTitleImg" mode="scaleToFill" />
          <view class="dialog-content-title">
            <view class="award-name" :class="[isFristAward ? 'db' : '']">
              {{ isFristAward ? '恭喜' : '' }}您获得了
              <p class="award-text">{{ prizeInfo.PrizeName }}</p>
            </view>
            <view v-if="!isFristAward" class="award-time">参与时间：{{ prizeInfo.Addtime }}</view>
          </view>
          <view class="pop__bg-center">
            <!-- 红包零钱 -->
            <view v-if="prizeInfo.IsGetHb" class="pop__prize-item" :style="{ backgroundImage: `url(${itemBg})` }">
            ...
            <!-- 券 -->
            <view v-if="prizeInfo.IsGetVolume" class="pop__prize-item" :style="{ backgroundImage: `url(${itemBg})` }">
              <image
              ...
              <view>
                ...
                <view class="prize-desc ellipsis-2">
                  ...
                  <text v-else-if="[0, 1, 2].indexOf(prizeInfo.VolumeType) > -1">
              ...
              <template v-else-if="prizeInfo.VolumeStatus === 0">
                <!-- #ifdef H5 -->
                <view class="prize-btn" @tap="handleContactUs">联系客服</view>
                <!-- #endif -->
                <!-- #ifdef MP-WEIXIN -->
                <button plain hover-class="none" class="prize-btn" open-type="contact">联系客服</button>
                <!-- #endif -->
              </template>
              <template v-else-if="prizeInfo.VolumeStatus == 1">
                ...
                <view v-else-if="prizeInfo.VolumeType === 3" class="prize-btn" @tap="handleGetCouponExplain">
                  如何解锁
                </view>
                <view v-else class="prize-btn" @tap="handleVolume">去使用</view>
              </template>
            </view>
            <!-- 积分 -->
            <view v-if="prizeInfo.Point" class="pop__prize-item" :style="{ backgroundImage: `url(${itemBg})` }">
            ...
            <!-- 参与机会 -->
            <view v-if="prizeInfo.IsGetDraw" class="pop__prize-item" :style="{ backgroundImage: `url(${itemBg})` }">
            ...
            <!-- 礼品 -->
            <view v-if="prizeInfo.IsProduct" class="pop__prize-item" :style="{ backgroundImage: `url(${itemBg})` }">
              ...
              <block v-else-if="prizeInfo.IsGetProduct && prizeInfo.IsVirtual === 2">
                <view v-if="[1, 4].includes(prizeInfo.ProductSendResult)" class="prize-btn" @tap="handleGoCard(false)">
                  去使用
                </view>
              </block>
              <view v-else-if="prizeInfo.IsGetProduct" class="prize-btn gray">已领取</view>
              <view
              ...
              <view v-else class="prize-btn">已失效</view>
            </view>
          </view>
        </view>
...
export default {
  ...
  data() {
    return {
      ...
      itemBg: staticURL(true) + 'ball/item-bg.png?v237',
  ...
  computed: {
    popTitleImg() {
      if (!this.winPage) return ''
      if (this.isFristAward) {
        return this.winPage.WinPageTop ? this.winPage.WinPageTop : staticURL(true) + 'ball/success-top.png'
      } else {
        return this.winPage.repeatAwardTop ? this.winPage.repeatAwardTop : staticURL(true) + 'ball/err-top.png'
      }
    },
    popBg() {
      if (!this.winPage) return ''
      if (this.isFristAward) {
        return this.winPage.WinPageBg ? this.winPage.WinPageBg : staticURL(true) + 'ball/win-bg.png'
      } else {
        return this.winPage.repeatAwardBg ? this.winPage.repeatAwardBg : staticURL(true) + 'ball/win-bg.png'
      }
    }
  },
  watch: {
...
.win-dialog-container {
  ...
  .content {
    ...
    .pop__content {
      ...
      justify-content: center;
      align-items: center;
      // background: linear-gradient(0deg, #ffffff 0%, #ffffff 65%, #ffe4ca 100%);
      border: 3rpx solid;
      border-radius: 10rpx;
