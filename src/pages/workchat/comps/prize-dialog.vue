    ...
    <view class="prize-dialog flex flex-column flex-middle flex-center">
      <template v-if="isPrize">
        <view class="title-box flex flex-middle">
          ...
          <view class="title">{{ info.isFirst ? '恭喜中奖' : '你已领过奖了' }}</view>
        ...
        <view class="prize-box">
          ...
          <image :src="info.prizeimage" class="prize-icon" />
        </view>
        <view class="sub-title fs-30">{{ info.prizename }}</view>
        <view class="prize-name fs-30">{{ info.trophy }}</view>
        <view class="btn-group">
          <view class="btn active" @tap="handleView">查看奖品</view>
          <view v-if="info.prizetype === 1" class="btn plain" @tap="handleGoChange">去兑换</view>
        </view>
      </template>
      <view v-else class="fail-box text-center">
        <image :src="PACKAGE_STATIC_URL + 'tap/fail-bg.png'" class="bg" />
        <view class="fail-tip fw-bold fs-36">很遗憾，你未中奖</view>
        <view class="btn-group flex flex-middle flex-center">
          <view class="btn plain" @tap="handleClose">关闭</view>
        </view>
      </view>
    </view>
  </u-mask>
