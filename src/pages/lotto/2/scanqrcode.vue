    ...
    handleWinActivityChanceByQrCode() {
      ...
      apiWinActivityChanceByQrCode(this.aid, this.tempKey, params)
        .then(res => {
          ...
          /* #ifdef H5 */
          uni.reLaunch({
            url: `/pages/lotto/2/index?aid=${this.aid}&memberlogin=${this.m}&skin=2&scansuccess=1`
          })
          /* #endif */
          /* #ifdef MP-WEIXIN */
          uni.navigateTo({
            url: `/packages/src/pages/lotto/2/index?aid=${this.aid}&memberlogin=${this.m}&skin=2&scansuccess=1`
          })
          /* #endif */
        })
        .catch(err => {
          /* #ifdef H5 */
          uni.reLaunch({
            url: `/pages/lotto/2/payout-fail?aid=${this.aid}&memberlogin=${this.m}&skin=2&errcode=${err.return_code}`
          })
          /* #endif */
          /* #ifdef MP-WEIXIN */
          uni.reLaunch({
            url: `/packages/src/pages/lotto/2/payout-fail?aid=${this.aid}&memberlogin=${this.m}&skin=2&errcode=${err.return_code}`
          })
          /* #endif */
        })
    },
