      ...
      >
        <template v-if="currentTab === 0">
          ...
          <view class="item">
            ...
            <u-parse class="rich-text" :html="info.ruletext" :tag-style="parseStyle" />
          </view>
        </template>
        <template v-if="currentTab === 1">
          ...
          <template v-else>
            <view v-for="(item, index) in list" :key="index" class="prize-item flex flex-middle">
              <image :src="PACKAGE_STATIC_URL + 'hb.png'" class="prize-icon" />
              <view class="flex-1 ml-20 mr-20 overflow-h">
                <view class="fs-28 ellipsis fw-bold">
                  ￥{{ item.awardamount | cashFixedTwo }}{{ item.prizetype | rewardTypwFilter }}
                  <text v-if="item.sendstatus === -1">（请联系客服补发）</text>
                </view>
                <view v-if="item.addtime" class="fs-24 mt-10 time">{{ item.addtime }}</view>
              </view>
            </view>
          </template>
        </template>
...
import { apiPageAwardRecords } from 'packages/api/promotion-gift'
