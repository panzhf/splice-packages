        ...
        <template v-if="currentTab === 1">
          ...
          <template v-else>
            <view v-for="(item, index) in list" :key="index" class="prize-item" @tap.stop="handleNavigate(item)">
              <view class="title-tag">第{{ item.PeriodOrder }}期</view>
              <text>集齐{{ item.MyChanceCount }}张拼图</text>
              <text>- {{ item.JackpotName }}</text>
...
<script>
import { apiGetMyPoolPrize } from 'packages/api/prizePool'
