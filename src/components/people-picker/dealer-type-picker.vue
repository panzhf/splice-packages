<template>
  <view class="people-picker fs-28">
    <u-popup v-model="showPicker" mode="bottom" border-radius="30" @close="$emit('input', false)">
      <view class="title tx-c">选择经销商类型</view>
      <scroll-view class="list" scroll-y="true" scroll-left="120" :show-scrollbar="false" @scrolltolower="scrollBottom">
        ...
        >
          ...
          <view v-if="dealertypeid == item.id" class="fs-28 primary-color mr-20">✔</view>
        </view>
...
import { apiGetDealerTypes } from 'packages/api/store'
export default {
  components: {},
  props: {
    value: Boolean,
  ...
  methods: {
    ...
    async getData() {
      if (this.busy) return
      this.busy = true
      let res = await apiGetDealerTypes({
        memberlogin: this.memberlogin //品牌商账号
      })
      this.busy = false
      if (this.$ck(res, true)) {
        this.list = res.return_data && res.return_data.list
      }
    },
